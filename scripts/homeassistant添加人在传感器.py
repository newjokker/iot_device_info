...
def add_sensor(client, config_topic):
    ...
    config_payload = {
        ...
        "state_topic": "txkj/jokker_desktop/person_occupancy",            # 传感器状态消息的主题，用于接收实际数据
        ...
        "device": {  
            "identifiers": ["txkj_jokker_desktop_occupancy_sensor_device"],     # 设备标识符，用于唯一识别设备
            "name": "OccupancySensor_LD2401​",                     # 设备名称
            "manufacturer": "CustomMQTTDevice",                         # 设备制造商信息
            "model": "LD2401"                                           # 设备型号
        },
        "availability_topic": "txkj/jokker_desktop/occupancy_status",  # 设备在线状态主题
...
def update_status(client, retain=True):
    # 上报在线状态
    client.publish("txkj/jokker_desktop/occupancy_status", "online", retain=retain)
...
def has_occupancy(client, occupied=True, retain=True):
    ...
    client.publish("txkj/jokker_desktop/person_occupancy", json.dumps({"state": state}), retain=retain)
...
if __name__ == "__main__":
    ...
    config_topic    = "homeassistant/binary_sensor/txkj_jokker_desktop_occupancy/config"
